...
connection.query(
    'CREATE OR REPLACE TABLE Vehicles (vehicle_id VARCHAR(255) PRIMARY KEY, vehicle_type VARCHAR(30) NOT NULL, model VARCHAR(255), emission int NOT NULL)',
    function(err, result){
        if(err) throw err;
...
// Create Route table
connection.query(
    'CREATE OR REPLACE TABLE Routes (route_id VARCHAR(255) PRIMARY KEY, user_id VARCHAR(255) NOT NULL, vehicle_id VARCHAR(255) NOT NULL, length int, CONSTRAINT fk_ruser_id FOREIGN KEY (user_ID) REFERENCES Users (user_ID), CONSTRAINT fk_vehicle_id FOREIGN KEY (vehicle_id) REFERENCES Vehicles (vehicle_id))',
    function(err, result){
        if(err) throw err;
